...
static void create_sample_lowtobig(struct list_head *head, element_t *space, int samples)
{
    printf("Creating low to big sample\n");
    for (int i = 0; i < samples; i++) {
        element_t *elem = space + i;
        elem->val = i;
        list_add_tail(&elem->list, head);
...
static void create_sample_bigtolow(struct list_head *head, element_t *space, int samples)
{
    printf("Creating big tp low sample\n");
    for (int i = 0; i < samples; i++) {
        element_t *elem = space + i;
        elem->val = samples-i;
        list_add_tail(&elem->list, head);
...
int main(void)
{
    ...
    struct list_head warmdata_head2, testdata_head2;
    int count;
    int nums = SAMPLES;
    ...
    element_t *warmdata2 = malloc(sizeof(*warmdata2) * SAMPLES);
    element_t *testdata2 = malloc(sizeof(*testdata2) * SAMPLES);

    create_sample_bigtolow(&sample_head, samples, nums);
    ...
    while (test2->impl) {
        printf("==== Testing %s ====\n", test2->name);
        /* Warm up */
        INIT_LIST_HEAD(&warmdata_head2);
        INIT_LIST_HEAD(&testdata_head2);
        copy_list(&sample_head, &testdata_head2, testdata2);
        copy_list(&sample_head, &warmdata_head2, warmdata2);
        test2->impl(&count, &warmdata_head2, compare);

        /* Test */
        count = 0;
        test2->impl(&count, &testdata_head2, compare);
        printf("  Comparisons:    %d\n", count);
        printf("  List is %s\n",
               check_list(&testdata_head2, nums) ? "sorted" : "not sorted");
        test2++;
    }
